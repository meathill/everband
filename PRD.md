...
12. Staff 维护轻量器材档案与当前持有人，访客通过动态二维码查看只读信息卡。

### 4.2 MVP 不包含

- 器材借还流程（check-out/check-in）、维修历史记录和责任人变更审批（当前只提供 §5.6 的轻量器材档案与只读信息卡）。
- Tutor/导师名册。
...
### 5.6 器材与资产管理（已交付轻量版，详见 §6.8）

轻量版：Staff 维护基础档案，公开侧只读展示，不做借还流程和维修记录。

| 对象 | 说明 |
| --- | --- |
| Asset | 组织持有的可追踪物品（乐器、制服等），挂一个二维码 |

Asset 字段：`id`、`organizationId`、`name`、`type`（如乐器/制服）、`serialNumber`（可选）、`currentHolderStudentId`（可空，Staff 手工维护的展示字段）、`notes`（自由文本，仅 Staff 可见）、`qrCodeId`（可空）、`status`、创建/更新/退役审计字段。
...
- 只能选择本组织的 active Student；学生转为非 active 后，Staff 侧提示复核，公开页停止展示其姓名。
- 公开页只展示组织名称、器材名称、类别、编号、脱敏后的当前持有人和组织联系邮箱；`notes` 永不公开。持有人姓名显示为“名字 + 姓氏首字母”，单段姓名只显示首字符加省略号。
- `qrCodeId` 为空表示器材已保存但二维码尚未生成。没有组织联系邮箱时允许保存器材，但不能生成公开二维码。
- 物品标记 `retired` 后本地 QrCode 转为 `disabled`；稳定公开路由继续显示统一下线提示，不能进入 404。恢复后健康的原二维码重新生效。
...
本版本会创建 `targetType = org_entry | asset` 的记录；`recruitment` 枚举值提前定义，待 §5.1 对应场景排期后再实际创建。

状态机：`active | disabled | broken`（`broken` 只表示 dyqr 明确返回短链不存在，需要 Staff 生成替代二维码；瞬时网络错误不会误判为损坏）。
...
- dyqr 平台账号的套餐配额是全部组织共享的。Everband 不固化套餐数量或单组织业务上限；套餐、总量或频率限制以 dyqr 的实际响应为准，并转为 Staff 可操作的错误提示。
...
1. Staff 创建器材记录，填写名称、自由文本类别、编号、内部 Notes，可选填本组织 active Student 作为当前持有人。
2. 系统保存器材后自动尝试生成 `QrCode`（目标类型 `asset`）；dyqr 失败不回滚器材，Staff 可稍后幂等重试。组织未配置联系邮箱时不能生成公开二维码。
3. Staff 下载 SVG/PNG 标签、打开公开卡片，并按单件刷新扫描数；统计注明最后同步时间。
4. 任何人扫码进入稳定的 `/a/:assetId` 只读详情页，看到器材公开字段、脱敏持有人和组织联系邮箱；Notes 与学生其他信息始终不公开。
5. Staff 更新当前持有人时，无需重新打印二维码，页面内容自动更新。
6. 物品退役时 Staff 将其标记为 `retired`，已打印标签进入统一停用页；恢复后健康的原二维码重新生效。
7. dyqr 明确返回短链不存在时将 QrCode 标为 `broken` 并允许生成替代二维码；瞬时不可用只提示重试。

完成标准：Staff 能保存、搜索、筛选、编辑、退役/恢复器材，二维码生成失败后可重试，已打印标签随资料和状态更新；扫码页面不提供任何写操作入口。

可见性要求：详情页对外公开，但只返回明确的字段白名单；Notes 不进入公开查询，当前持有人只显示脱敏姓名，不能关联展示 group、联系方式等学生信息。
...
9. Staff 创建器材、分配持有人、下载二维码并验证公开字段白名单；退役后标签停用，恢复后重新可见，Parent 无管理入口且不能直接访问。
...
- 成员招募流程已在本次设计中确定（§4.3、§5.1、§6.7），但尚未进入交付范围；器材轻量版已按 §5.6/§6.8 交付，完整借还与维修流程仍属后续扩展。
